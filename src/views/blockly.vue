        ...
        <div class="output-section">
          ...
          <div class="joint-display">
            ...
            <div class="joint-values-horizontal">
              <div
                v-for="(value, index) in jointValues"
                :key="index"
                class="joint-item"
              >
...
const createCustomBlocks = () => {
  Blockly.Blocks["robot_start"] = {
    init: function () {
      this.appendDummyInput().appendField("🔰 开始程序");
  ...
  Blockly.Blocks["print_joints"] = {
    init: function () {
      this.appendDummyInput().appendField("📋 输出关节角度");
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setColour(120);
      this.setTooltip("输出当前设置的关节角度");
    },
  };
  ...
  if (currentExecutingBlock) {
    ...
    if (block && block.getSvgRoot()) {
      ...
      if (blockPath && blockPath.hasAttribute("data-original-fill")) {
        ...
        blockPath.setAttribute(
          "fill",
          blockPath.getAttribute("data-original-fill")
        );
  ...
  if (workspace) {
    try {
      const startBlocks = workspace
        .getTopBlocks(true)
        .filter((block) => block.type === "robot_start");
...
const executeCode = async () => {
  ...
  try {
    ...
    if (generatedCode.value && !generatedCode.value.startsWith("//")) {
      try {
        const codeLines = generatedCode.value
          .split("\n")
